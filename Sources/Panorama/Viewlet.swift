...
open class Viewlet {
    ...
    public func drawText(
        ...
        switch verticalAlignment {
        case .top:
            textRect = rect.divided(atDistance: suggestedSize.height, from: .maxYEdge).slice
        case .center:
            let topMargin = (rect.size.height - suggestedSize.height) * 0.5
            let centeredRect = rect.insetBy(dx: 0, dy: topMargin)
            textRect = CGRect(x: rect.minX, y: centeredRect.minY, width: rect.width, height: suggestedSize.height)
        case .bottom:
            textRect = rect.divided(atDistance: suggestedSize.height, from: .minYEdge).slice
        }
